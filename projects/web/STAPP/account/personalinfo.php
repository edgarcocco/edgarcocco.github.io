...
if(isset($_POST['submit']))
{
	...
	else if(str_contain_space($username))
	{
		...
		if(!userNameExists($username))
		{
			...
			$query = "UPDATE " . $table . " SET username='$username', phone_number='$phonenumber',
...
if($success==true)
{
	echo '<div class="alert alert-success text-center container m-5">';
	echo '<h1>Registered completed successfully!</h1>';
	echo '<p>You may want to proceed to your <a href="../user/profile">profile</a></p>';
	echo '</div>';
}
else {
?>

<div class="container text-center w-50 my-2">
	<p class="lead">We need some more information, to complete your registration!</p>
</div>

<div class="container border rounded p-4 my-3 w-50">
	<?php
		if(!empty($error_msg)){
	...
	<form action="" method="post">
		...
		<?php
			$dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
			$user_id = $_SESSION['user_id'];
			$query = "SELECT * FROM stapp_user WHERE user_id='$user_id'";
		...
		<button type="submit" name="submit" class="btn btn-success">Complete register</button>
	</form>
</div>
